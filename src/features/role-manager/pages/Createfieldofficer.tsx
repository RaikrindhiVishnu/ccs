...
const CreateFieldOfficer = () => {
  ...
  const handleCreateFieldOfficer = async () => {
    try {
      const payload = {
      ...
      console.log("Field Officer Created:", response);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[color:var(--surface-page)] rounded-[2rem] px-[clamp(1.5rem,6.81vw,6.8125rem)] py-[clamp(1.5rem,2.64vw,2.375rem)]">
      ...
      grid
      grid-cols-3
      gap-x-[clamp(0.875rem,1.4vw,1.5rem)]
      gap-y-[clamp(0.75rem,1vw,1rem)]
      w-full
    "
            >
              <FormDropdown
          ...
          >
            ...
            <Button
              ...
              onClick={handleCreateFieldOfficer}
              disabled={isLoading}
              className={cn(
                "w-[10.5625rem]",
                "h-[2.5rem]",
