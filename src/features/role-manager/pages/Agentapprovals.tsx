...
export const AgentApprovalsPage = () => {
  ...
  const fetchPendingAgents = async (statusId: number, dates?: { startDate: string; endDate: string } | null) => {
    try {
      ...
      if (dates) {
        ...
        queryParams.start_date = dates.startDate;
        queryParams.end_date = dates.endDate;
      }
      const response = await getAllAgents(queryParams).unwrap();


      const apiAgents = response?.data || [];

      const formattedAgents: Agent[] = apiAgents.map((item: any) => {
        const name = item.name || item.full_name || `${item.first_name || ""} ${item.last_name || ""}`.trim();
        const first = item.first_name || "";
        const last = item.last_name || "";
        const initials = first || last
          ? `${first} ${last}`.split(" ").map((word: string) => word[0]).join("").toUpperCase()
          : "NA";
            ...
            <div className="relative">
              <button
                ...
                className="
                  ...
                  px-[1.25rem] py-[0.5rem] gap-2
                  bg-white border border-[#EBEBEB]
                  shadow-[0px_37.4311px_15.1507px_rgba(0,0,0,0.01)]
                  rounded-[64.665px]
                  font-[family-name:var(--font-sans)] font-normal text-[0.875rem]
                  text-[#5A5C5E] cursor-pointer
                  min-w-[120px] h-[2.75rem] lg:h-[3rem] xl:h-[3.25rem]
                  hover:bg-gray-50 transition-colors
                "
              >
                <span className="capitalize">
                  {filterType === "today" ? "Today" : filterType === "month" ? "This Month" : "Custom"}
                </span>
                <svg
                  width="12"
                  height="12"
              ...
              {dropdownOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-1.5 bg-white border border-[#EBEBEB] rounded-xl shadow-lg py-1.5 z-40 w-40">
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className={cn(
                        "w-full text-left px-4 py-2 text-sm cursor-pointer bg-transparent border-none font-sans",
                        filterType === "today" ? "text-[color:var(--brand-500)] font-semibold" : "text-gray-700 hover:bg-gray-50"
                      )}
                    >
                      Today
                    </button>
                    <button
                      ...
                      className={cn(
                        "w-full text-left px-4 py-2 text-sm cursor-pointer bg-transparent border-none font-sans",
                        filterType === "month" ? "text-[color:var(--brand-500)] font-semibold" : "text-gray-700 hover:bg-gray-50"
                      )}
                    >
                      This Month
                    </button>
                    <button
                      ...
                      className={cn(
                        "w-full text-left px-4 py-2 text-sm cursor-pointer bg-transparent border-none font-sans",
                        filterType === "custom" ? "text-[color:var(--brand-500)] font-semibold" : "text-gray-700 hover:bg-gray-50"
                      )}
                    >
                      Custom Range
                    </button>
