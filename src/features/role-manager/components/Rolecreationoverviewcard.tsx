        ...
        >
          ...
          <div
            className={cn(
              ...
              "pb-[clamp(0.875rem,1.2vw,1.25rem)]",
            )}
          >
            {yTicks.map((v) => (
              <Typography
          ...
          <div className="flex-1 flex flex-col min-h-0">
            {/* Bar columns */}
            <div className="flex-1 flex items-end gap-[clamp(0.25rem,0.5vw,0.625rem)] min-h-0 overflow-hidden">
              {data.map((day) => {
                const total = day.ro + day.io + day.fo + day.agents;
                // Cap at 100% so bars never exceed the chart container height
                ...
                return (
                  ...
                  >
                    <div
                      className="w-full flex flex-col gap-[0.125rem] overflow-hidden"
                      style={{
                        height: `${totalPct}%`,
                        // FIX: guarantee a visible minimum height when data exists
                        // prevents near-zero values (e.g. 1 out of 1500) from collapsing to 0px
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              // FIX: slightly larger min-h so segments are visible
                              // even when the outer wrapper is at its minimum height
                              val > 0 ? "min-h-[0.375rem]" : "",
                            )}
                            style={{
                              flex: `${val} 0 0`,
                              background: s.color,
