  ...
  return (
    <div className="flex flex-col flex-1 w-full h-auto min-h-[417px] relative">
      ...
      <div className="relative w-full">
        {/* Bars container with fixed height so % heights work */}
          ...
          {chartData.map((data, index) => {
            ...
            return (
              <div key={index} className="flex flex-col items-center justify-end flex-1 h-full">
                ...
                {isActive && (
                  <div className="mb-2 bg-[#202020] text-white text-xs font-inter py-1 px-2.5 rounded-lg whitespace-nowrap">
        ...
        <div className="flex items-center justify-between gap-1 w-full px-1 mt-3">
          {chartData.map((data, index) => (
            <span
              key={index}
              className="flex-1 text-center text-[#6F6F6F] text-sm font-normal font-inter capitalize"
            >
              {data.day}
            </span>
          ))}
