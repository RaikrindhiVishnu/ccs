...
const FLOW_STEPS = [
  {
    ...
    routePath: 'customer-information',
    nextStepId: 'legal-documents',
    tabs: [],
  },
  {
    id: 'legal-documents',
    name: 'LEGAL DOCUMENTS',
    routePath: 'legal-documents',
    nextStepId: 'agriculture-report',
    tabs: [
      { id: 'land-document', label: 'Land Document', status: 'pending' },
      { id: 'pattadhar-passbook', label: 'Pattadhar Passbook', status: 'pending' },
      { id: 'link-document', label: 'Link Document', status: 'pending' },
      { id: 'kasara-pahani', label: 'Kasara Pahani & Proceeding Copies', status: 'pending' },
      { id: 'revenue-record', label: 'Revenue Record', status: 'pending' },
      { id: 'lease-agreement', label: 'Lease Agreement', status: 'pending' },
      { id: 'death-certificate', label: 'Death Certificate', status: 'pending' },
      { id: 'partition-deed', label: 'Partition Deed', status: 'pending' },
      { id: 'encumbrance-certificate', label: 'Encumbrance Certificate', status: 'pending' },
      { id: 'land-coordinates', label: 'Land Coordinates', status: 'pending' },
      { id: 'owner-video-kyc', label: 'Owner Video KYC', status: 'pending' },
    ] as DocumentTab[],
  },
  {
    id: 'agriculture-report',
    name: 'AGRICULTURE REPORT',
    routePath: 'agriculture-report',
    nextStepId: 'land-boundaries',
    tabs: [
      { id: 'soil-test', label: 'Soil Test Report', status: 'pending' },
      { id: 'water-test', label: 'Water Test Report', status: 'pending' },
      { id: 'yield-estimation', label: 'Yield Estimation', status: 'pending' },
    ] as DocumentTab[],
  },
  {
    id: 'land-boundaries',
    name: 'LAND & BOUNDARIES',
    routePath: 'land-boundaries',
    nextStepId: 'valuation',
    tabs: [
  ...
  },
  {
    ...
    name: 'VALUATION',
    routePath: 'valuation',
    nextStepId: 'local-intelligence',
    tabs: [
      { id: 'valuation-report', label: 'Valuation Report', status: 'pending' },
      { id: 'market-analysis', label: 'Market Analysis', status: 'pending' },
    ] as DocumentTab[],
  },
  {
    id: 'local-intelligence',
    name: 'LOCAL INTELLIGENCE',
    routePath: 'local-intelligence',
...
const VerificationFlow: React.FC = () => {
  ...
  const isCustomerInfo = currentPathSegment === 'customer-information';
  
  // Find which step we are on (default to legal-documents if not found)
  const currentStep = FLOW_STEPS.find(s => location.pathname.includes(s.routePath)) 
    || FLOW_STEPS.find(s => s.id === 'legal-documents')!;
  
  const nextStep = FLOW_STEPS.find(s => s.id === currentStep.nextStepId);

  // State
  const [tabs, setTabs] = useState<DocumentTab[]>(currentStep.tabs);
  ...
  const handleApprove = () => {
    const updatedTabs = tabs.map((t) => t.id === activeTabId ? { ...t, status: 'approved' } : t);
  ...
  const handleRejectConfirm = (reason: string) => {
    setTabs((prev) =>
      prev.map((t) => t.id === activeTabId ? { ...t, status: 'rejected' } : t)
    );
    setShowRejectModal(false);
  };
  ...
  // Capitalize nicely for breadcrumbs
    ...
    <div className="bg-[#1A1A1A] min-h-screen relative flex flex-col items-center overflow-hidden font-sans pt-4">
      ...
      >
        <span className="text-gray-400 font-semibold text-[13px] tracking-wide">
          Super Admin / {currentStepTitleCase} {currentTab ? `/ ${currentTab.label}` : ''}
        </span>
      </div>
