...
export const UploadShortTimelineSidebar: React.FC<UploadShortTimelineSidebarProps> = ({
  ...
  return (
    <div
      className={cn(
        "relative box-border bg-white rounded-[clamp(1rem,1.67vw,1.8rem)] shadow-[0px_20px_40px_rgba(0,49,50,0.06)] p-[clamp(1.25rem,2.08vw,2rem)] flex flex-col justify-between select-none shrink-0 w-full lg:w-[clamp(20rem,28.47vw,28rem)] h-auto lg:h-[clamp(22rem,30.76vw,30rem)]",
        className
      )}
      style={style}
    >
      ...
      <div className="relative flex flex-col justify-between py-1 h-[clamp(6.5rem,_9.86vw,_10.5rem)] mt-[clamp(0.5rem,_0.694vw,_1rem)] mb-[clamp(0.5rem,_0.694vw,_1rem)] pl-[clamp(3rem,_4.72vw,_5.5rem)]">
        {/* Timeline Vertical Line (Line 495) */}
        <div className="absolute border-l border-[rgba(44,44,44,0.25)] left-[clamp(3.5rem,_5.15vw,_6rem)] top-[clamp(0.4rem,_0.625vw,_0.8rem)] h-[clamp(5.5rem,_8.4vw,_9.5rem)] w-0" />
        ...
        <div className="relative flex flex-row items-center gap-[clamp(0.9rem,1.46vw,1.8rem)] z-10">
          ...
          <span
            className={`font-['Plus_Jakarta_Sans'] font-semibold uppercase tracking-wider leading-none text-left ${
              isTopActive
                ? "text-[clamp(0.95rem,_1.25vw,_1.3rem)] text-[#2A3008]"
                : "text-[clamp(0.75rem,_0.972vw,_1.1rem)] text-[#5A5C5E]"
            }`}
          >
            {topStep.label}
          </span>
      ...
      <div className="flex flex-row justify-between items-center w-full h-[clamp(0.9rem,_1.25vw,_1.3rem)] mt-[clamp(0.5rem,_0.694vw,_1rem)]">
        {/* Previous Button */}
        <button
          type="button"
