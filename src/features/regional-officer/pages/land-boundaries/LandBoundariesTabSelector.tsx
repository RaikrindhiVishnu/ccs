...
interface LandBoundariesTabSelectorProps {
  subTabs: string[];
  activeSubTab: number;
...
export const LandBoundariesTabSelector: React.FC<LandBoundariesTabSelectorProps> = ({
  subTabs,
  activeSubTab,
  setActiveSubTab,
  completedSubTabs
  ...
  return (
    <div className="submit-form-right-card">
      <div className="boundaries-tabs-container">
        {/* Row 1: Land Images, Landscape View of Farmlands, Shape of the Land */}
        <div className="boundaries-tabs-row">
          {subTabs.slice(0, 3).map((tabName, idx) => {
            ...
            const widths = [168, 283, 203];
            return (
              <div
                key={globalIdx}
                onClick={() => setActiveSubTab(globalIdx)}
        ...
        {/* Row 2: Water and Electricity Facility, Any Existing Trees, Master Plan */}
        <div className="boundaries-tabs-row">
          {subTabs.slice(3, 6).map((tabName, idx) => {
            const globalIdx = idx + 3;
            const isActive = globalIdx === activeSubTab;
            const isCompleted = completedSubTabs.includes(globalIdx);
            const widths = [273, 198, 178];
            return (
              <div
                key={globalIdx}
                onClick={() => setActiveSubTab(globalIdx)}
        ...
        {/* Row 3: Survey Report, East Boundaries, West Boundaries, North Boundaries */}
        <div className="boundaries-tabs-row">
          {subTabs.slice(6, 10).map((tabName, idx) => {
            const globalIdx = idx + 6;
            const isActive = globalIdx === activeSubTab;
            const isCompleted = completedSubTabs.includes(globalIdx);
            const widths = [191, 191, 197, 201];
            return (
              <div
                key={globalIdx}
                onClick={() => setActiveSubTab(globalIdx)}
        ...
        {/* Row 4: South Boundaries */}
        <div className="boundaries-tabs-row">
          {subTabs.slice(10, 11).map((tabName, idx) => {
