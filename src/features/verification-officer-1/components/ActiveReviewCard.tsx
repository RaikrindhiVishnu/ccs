...
export const ActiveReviewCard = () => {
  ...
  const { id, title, subtitle, assignee, progress, buttonText } = dashboardMockData.activeReview;

  return (
    <div className="bg-[#F7F8FA] rounded-[30px] p-6 md:p-8 w-full md:max-w-[423px] h-auto min-h-[417px] flex flex-col shadow-sm relative border border-gray-100">
      {/* Top Labels */}
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-auto">
        ...
        <div className="bg-[#F1F1F1] rounded-[15px] p-4 flex-1 flex flex-col gap-2">
          <span className="text-[#6E6E6E] text-xs font-normal font-inter">
            {assignee.role}
          </span>
          <div className="flex items-center gap-3 mt-1">
            ...
            <div className="flex flex-col">
              <span className="text-black text-xs font-semibold font-inter leading-tight">
                {assignee.name}
              </span>
              <span className="text-[#6E6E6E] text-[10px] font-normal font-inter mt-0.5">
                ID : {assignee.id}
              </span>
            </div>
          </div>
        ...
        <div className="bg-[#F1F1F1] rounded-[15px] p-4 flex-1 flex flex-col justify-between">
          ...
          <div className="bg-[#C6DB83]/60 rounded-[10px] w-full h-8 flex relative overflow-hidden mt-2">
            ...
            >
              <span className="text-[#202020] text-xs font-bold font-inter relative z-10">
                {progress}%
              </span>
            </div>
          </div>
      ...
      {/* Resume Button */}
      <button 
        onClick={() => navigate(`/verification-officer-1/assigned-farmlands-owner-details/${encodeURIComponent(id)}`)}
        className="w-full h-[45px] mt-6 bg-[#2780C4] hover:bg-[#2069A1] rounded-[25px] flex justify-center items-center text-white text-base font-bold font-plus-jakarta hover:opacity-90 transition-all border-none cursor-pointer"
      >
        {buttonText}
      </button>
    </div>
