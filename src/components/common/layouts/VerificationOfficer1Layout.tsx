...
import { VERIFICATION_OFFICER_1_NAV_ITEMS } from "@/features/verification-officer-1/data/navigation";

export const VerificationOfficer1Layout = () => {
  const navigate = useNavigate();
  const location = useLocation();

  // Determine active item based on current path
  const activeItem = VERIFICATION_OFFICER_1_NAV_ITEMS.find(item => location.pathname.includes(item.path))?.id || "dashboard";
    ...
    <div className="min-h-screen bg-[#EBEBEB] flex flex-col font-plus-jakarta pb-12 layout-container-zoom">
      ...
      >
        ...
        <button
          ...
          className="flex items-center justify-center border-none bg-transparent cursor-pointer ml-4 hover:opacity-85 transition-opacity"
        >
          <img
            src={glcLogo}
        ...
        <div className="flex items-center gap-[13px] mr-2">
          <button className="w-[52px] h-[52px] rounded-full flex items-center justify-center relative hover:bg-gray-50 transition-colors cursor-pointer border-none bg-transparent">
...
export default VerificationOfficer1Layout;
