...
export default function ScreeningChart() {
  ...
  const SVG_H = 160, barW = 8, overlap = 8;
  const SVG_W = approved.length * 28;

  return (
    <div
      className="
        flex flex-col rounded-[1.5rem] bg-[var(--chart-bg)]
        px-[1.25rem] py-[1rem]
    ...
    >
      ...
      <div className="mt-3 flex min-h-0 flex-1 items-stretch gap-4 xl:mt-4 xl:gap-5">
        <div className="min-h-0 flex-1">
          <svg width="100%" height="100%" viewBox={`0 0 ${SVG_W} ${SVG_H + 14}`} preserveAspectRatio="xMidYMid meet">
            {approved.map((_, i) => {
              const x  = i * 28 + 4;
